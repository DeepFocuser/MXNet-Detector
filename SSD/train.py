    ...
    for i in tqdm(range(start_epoch + 1, epoch + 1, 1), initial=start_epoch + 1, total=epoch):
        ...
        for batch_count, (image, _, cls_all, box_all, _) in enumerate(train_dataloader, start=1):
            ...
            with autograd.record(train_mode=True):
                ...
                for image_split, cls_split, box_split in zip(image, cls_all, box_all):
                    ...
                    cls_split = gluon.utils.split_and_load(cls_split, ctx_list, even_split=False)
                    box_split = gluon.utils.split_and_load(box_split, ctx_list, even_split=False)
                    ...
                    for img, cls_target, box_target in zip(image_split, cls_split, box_split):
                        ...
                        if classHardNegativeMining:
                            ...
                            conf_loss = -mx.nd.pick(pred, cls_target, axis=-1)  # (batch, all feature number)
                            '''
                            ...
                            conf_loss = confidence_loss(cls_pred, cls_target, positive_samples.expand_dims(axis=-1))
                            if positive_numbers:
                                conf_loss = mx.nd.divide(conf_loss, positive_numbers)
                            else:
                                conf_loss = mx.nd.multiply(conf_loss, 0)
                        ...
                        if boxHardNegativeMining:
                            ...
                            pred = mx.nd.log_softmax(cls_pred, axis=-1)
                            negative_samples = 1 - positive_samples
                            conf_loss_for_box = -mx.nd.pick(pred, cls_target, axis=-1)  # (batch, all feature number)
        ...
        if i % eval_period == 0 and valid_list:
            ...
            for image, label, cls_all, box_all, _ in valid_dataloader:
                ...
                cls_all = gluon.utils.split_and_load(cls_all, ctx_list, even_split=False)
                box_all = gluon.utils.split_and_load(box_all, ctx_list, even_split=False)
