...
from core.utils.dataprocessing.transformer import RetinaTrainTransform, RetinaValidTransform
...
def traindataloader(multiscale=False, factor_scale=[8, 5], augmentation=True, path="Dataset/train",
    ...
    if multiscale:
        ...
        train_transform = [RetinaValidTransform(input_size[0], input_size[1], net=net, mean=mean, std=std,
                                                foreground_iou_thresh=foreground_iou_thresh,
                                                background_iou_thresh=background_iou_thresh,
...
def validdataloader(path="Dataset/valid", input_size=(512, 512),
                    ...
                    make_target=False):
    transform = RetinaValidTransform(input_size[0], input_size[1], net=net, mean=mean, std=std,
                                     foreground_iou_thresh=foreground_iou_thresh,
                                     background_iou_thresh=background_iou_thresh,
                                     make_target=make_target)
...
def testdataloader(path="Dataset/test", input_size=(512, 512),
    ...
    transform = RetinaValidTransform(input_size[0], input_size[1], mean=mean,
