...
class AnchorNet(HybridBlock):
    ...
    def __init__(self, net=None, version=None, target_size=None,
        ...
        with self.name_scope():
            ...
            self._fpn_resnet = get_fpn_resnet(version, ctx=ctx, dummy=True)
            self._fpn_resnet.forward(mx.nd.ones(shape=(1, 3) + target_size, ctx=ctx))
