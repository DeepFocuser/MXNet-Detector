...
from core.utils.dataprocessing.transformer import RetinaTrainTransform, RetinaTrainTransform_V0, RetinaTrainResize_V0
...
def traindataloader(multiscale=False, factor_scale=[8, 5], augmentation=True, path="Dataset/train",
                    image_normalization=True, box_normalization=True,
                    ...
                    background_iou_thresh=0.4, make_target=True):
    if multiscale:
        ...
        if augmentation:
            train_transform = [RetinaTrainTransform_V0(x * h_seed, x * w_seed, net=net, mean=mean, std=std,
                                                       foreground_iou_thresh=foreground_iou_thresh,
                                                       background_iou_thresh=background_iou_thresh,
                                                       make_target=make_target) for x in
                               range(init, end)]
        else:
            train_transform = [RetinaTrainResize_V0(x * h_seed, x * w_seed, net=net, mean=mean, std=std,
                                                    foreground_iou_thresh=foreground_iou_thresh,
                                                    background_iou_thresh=background_iou_thresh,
                                                    make_target=make_target) for x in
                               range(init, end)]
        ...
        if augmentation:
            train_transform = RetinaTrainTransform(input_size[0], input_size[1], net=net,
                                                   foreground_iou_thresh=foreground_iou_thresh,
                                                   background_iou_thresh=background_iou_thresh,
...
def validdataloader(path="Dataset/valid", image_normalization=True, box_normalization=True, input_size=(512, 512),
...
if __name__ == "__main__":
    ...
    dataloader, dataset = validdataloader(path=os.path.join(root, 'Dataset', 'valid'),
                                          image_normalization=False,
                                          box_normalization=False,
                                          input_size=(320, 640), batch_size=8, num_workers=0, shuffle=True)
