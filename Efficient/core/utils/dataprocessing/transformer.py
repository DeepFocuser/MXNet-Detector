...
# for multiscale
class EfficientTrainResize_V0(object):
    ...
    def __call__(self, img, bbox, name):
        ...
        if self._make_target:
            ...
            cls_target, box_target = self._target_generator(self._anchor, bbox[:, :, :4],
                                                            bbox[:, :, 4:5])
            return img, cls_target[0], box_target[0], name
        else:
            return img, bbox, name


# for multiscale
class EfficientTrainTransform_V0(object):
...
class EfficientTrainTransform(object):

    def __init__(self, height, width, net=None, foreground_iou_thresh=0.5, background_iou_thresh=0.4, make_target=True):
    ...
    def __call__(self, img, bbox):
        ...
        if self._make_target:
            ...
            cls_target, box_target = self._target_generator(self._anchor, bbox[:, :, :4],
                                                            bbox[:, :, 4:5])
            return img, cls_target[0], box_target[0]
        else:
            return img, bbox
