...
class Efficient(HybridBlock):
    ...
    def __init__(self, version=0,
        ...
        with self.name_scope():
            ...
            for _ in range(repeat[version]):
                self._class_subnet.add(ConvPredictor(num_channel=channels[version],
                                                     kernel=(3, 3),
                                                     pad=(1, 1),
                                                     stride=(1, 1),
                                                     activation='relu',
                ...
                self._box_subnet.add(ConvPredictor(num_channel=channels[version],
                                                   kernel=(3, 3),
                                                   pad=(1, 1),
                                                   stride=(1, 1),
                                                   activation='relu',
            ...
            logging.info(f"{self.__class__.__name__}_{version} Head weight init 완료")
    ...
    def hybrid_forward(self, F, x):
        ...
        cls_preds = [F.flatten(data=F.transpose(data=self._class_subnet(bifpn_feature), axes=(0, 2, 3, 1)))
                     for bifpn_feature in self._bifpn(x)]
        # (batch, height, width, 4) -> (batch, -1)
        box_preds = [F.flatten(data=F.transpose(data=self._box_subnet(bifpn_feature), axes=(0, 2, 3, 1)))
                     for bifpn_feature in self._bifpn(x)]
