...
from core.utils.dataprocessing.transformer import EfficientTrainTransform, EfficientTrainResize
...
def traindataloader(multiscale=False, factor_scale=[8, 5], augmentation=True, path="Dataset/train",
    ...
    if multiscale:
        ...
        if augmentation:
            train_transform = [EfficientTrainTransform(x * h_seed, x * w_seed, net=net, mean=mean, std=std,
                                                       foreground_iou_thresh=foreground_iou_thresh,
                                                       background_iou_thresh=background_iou_thresh,
                                                       make_target=make_target) for x in
                               range(init, end)]
        else:
            train_transform = [EfficientTrainResize(x * h_seed, x * w_seed, net=net, mean=mean, std=std,
                                                    foreground_iou_thresh=foreground_iou_thresh,
                                                    background_iou_thresh=background_iou_thresh,
                                                    make_target=make_target) for x in
                               range(init, end)]
        ...
        if augmentation:
            train_transform = [EfficientTrainTransform(input_size[0], input_size[1], net=net, mean=mean, std=std,
                                                       foreground_iou_thresh=foreground_iou_thresh,
                                                       background_iou_thresh=background_iou_thresh,
                                                       make_target=make_target)]
        else:
            train_transform = [EfficientTrainResize(input_size[0], input_size[1], net=net, mean=mean, std=std,
                                                    foreground_iou_thresh=foreground_iou_thresh,
                                                    background_iou_thresh=background_iou_thresh,
                                                    make_target=make_target)]
