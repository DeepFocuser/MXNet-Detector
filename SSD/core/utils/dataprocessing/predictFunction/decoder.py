...
class ClassMDecoder(HybridBlock):
    ...
    def __init__(self, num_classes=None, thresh=0.01, from_softmax=False):
    ...
    def hybrid_forward(self, F, cls_preds):
        if not self._from_softmax:
            cls_preds = F.softmax(cls_preds, axis=-1)
        cls_preds = cls_preds.slice_axis(axis=-1, begin=1, end=None)  # batch x all feature number x foreground class
...
if __name__ == "__main__":
    from core import SSD_VGG16, DetectionDataset_V1
    import os
    ...
    # classdecoder = ClassMDecoder(num_classes=num_classes, thresh=0.01, from_softmax=False)
    classdecoder = ClassMPDecoder(num_classes=num_classes, thresh=0.01, from_softmax=False)
    ...
    '''
    class id shape : (1, 24564, 5)
    class scores shape : (1, 24564, 5)
    box predictions shape : (1, 24564, 4)
    '''
